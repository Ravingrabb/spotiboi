...
class UserSettings:
    ...
    def get_several_playlists_data(self, playlist_ids: list):
        ...
        output = tuple(
            self.spotify.playlist(playlist.playlist_id, fields="name, images, external_urls, id") for playlist in
            playlist_ids)
        return output
    ...
    def attach_playlist(self, query, scheduler):
        ...
        try:
            ...
            if query.playlist_id and \
                    self.spotify.playlist_is_following(query.playlist_id, [self.user_query.spotify_id])[0]:
            ...
            # если ID плейлиста привязан, но юзер не подписан на плейлист
            elif query.playlist_id and not \
                    self.spotify.playlist_is_following(query.playlist_id, [self.user_query.spotify_id])[0]:
                ...
                if query.job_id in scheduler:
                    ...
                    app.logger.error(
                        f'Traceback: отключено автообновление у клиента, так как нет подписки на плейлист. {query.playlist_id}. Юзер {self.user_query.spotify_id}')
        except spotipy.exceptions.SpotifyException as e:
            app.logger.error(
                'Tasks -> Attach_playlist -> spotipy.exceptions.SpotifyException. Неправильно указан Playlist ID. Если ошибка будет поймана вновь - выводить её юзеру, а не в консоль')
